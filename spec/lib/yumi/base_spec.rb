...
describe Yumi::Base do
  ...
  describe '#as_relationship' do
    it 'generates the relationship hash' do
      expect(Fake::GamePresenter.new(url, resources, Fake).as_relationship).to eq(data: [{ type: 'games', id: '1' }, { type: 'games', id: '2' }],
                                                                         links: {
    ...
    context 'with a prefix' do
      it 'generates the relationship hash' do
        expect(Fake::GamePresenter.new(url, resources, Fake, 'cool_prefix/').as_relationship).to eq(data: [{ type: 'games', id: '1' }, { type: 'games', id: '2' }],
                                                                                           links: {
  ...
  describe '#as_included' do
    it 'generates the included hash' do
      expect(Fake::GamePresenter.new(url, resource, Fake).as_included).to eq(type: 'games',
                                                                    id: '1',
                                                                    attributes: {
                                                                      title: 'Super Game',
                                                                      rating: '10'
